...
describe("Application", () => {
  ...
  it("loads data, edits an interview and keeps the spots remaining for Monday the same", async () => {
    ...
    // 4. Check that the name "Archie Cohen" is displayed
    await waitForElement(() => getByDisplayValue(appointment, "Archie Cohen"));

    // 5. Change text from Archie Cohen to Lydia Miller-Jones
    fireEvent.change(getByDisplayValue(appointment, "Archie Cohen"), {
      target: { value: "Lydia Miller-Jones" },
    });
    // 6. Click on interviewer Sylvia Palmer
    fireEvent.click(getByAltText(appointment, "Sylvia Palmer"));
    // 7. Click save
    fireEvent.click(getByText(appointment, "Save"));
    // 8. Expect the text saving and wait for the element Lydia Miller Jones
    expect(getByText(appointment, "Saving")).toBeInTheDocument();
    await waitForElement(() => getByText(appointment, "Lydia Miller-Jones"));
    // 9. Check that there is one spot remaining
    const day = getAllByTestId(container, "day").find((day) => queryByText(day, "Monday"));
